...
class displayNephrology extends StatelessWidget {
  ...
  Stream<QuerySnapshot> getUsersStreamSnapshots(BuildContext context) async* {
    ...
    yield* Firestore.instance.collection('Users').document(uid.uid).collection('Nephrology').snapshots();
  }
  ...
  Widget buildCard(BuildContext context, DocumentSnapshot data) {
    controllerPrevious = data['Surgery'];
  ...
  Future<void> _createPDF() async {
    ...
    page.graphics.drawString('Nephrology', PdfStandardFont(PdfFontFamily.helvetica, 25, style: PdfFontStyle.bold));
    ...
    page.graphics.drawString('+ Previous Surgeries : $controllerPrevious', PdfStandardFont(PdfFontFamily.timesRoman, 15), bounds: const Rect.fromLTWH(0, 60, 0, 0));

    //heading
    page.graphics.drawString('Current Problems', PdfStandardFont(PdfFontFamily.timesRoman, 15, style: PdfFontStyle.bold), bounds: const Rect.fromLTWH(0, 90, 0, 0));
    //with heading start with (15,0,0,0)
    page.graphics.drawString('1. Problems/Complains : $controllerNewProblem', PdfStandardFont(PdfFontFamily.timesRoman, 12), bounds: const Rect.fromLTWH(15, 120, 0, 0));
    page.graphics.drawString('2. Symptoms : $controllerSymptoms', PdfStandardFont(PdfFontFamily.timesRoman, 12), bounds: const Rect.fromLTWH(15, 150, 0, 0));
    page.graphics.drawString('3. Treatment/Medications : $controllerMedications', PdfStandardFont(PdfFontFamily.timesRoman, 12), bounds: const Rect.fromLTWH(15, 180, 0, 0));

    List<int> bytes = document.save();
    document.dispose();

    Auth().saveAndLaunchFile(bytes, 'Nephrology.pdf');
  }
}
