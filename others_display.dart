...
class displayOthers extends StatelessWidget {
  ...
  Stream<QuerySnapshot> getUsersStreamSnapshots(BuildContext context) async* {
    ...
    yield* Firestore.instance.collection('Users').document(uid.uid).collection('Others').snapshots();
  }
  ...
  Future<void> _createPDF() async {
    ...
    page.graphics.drawString('Others', PdfStandardFont(PdfFontFamily.helvetica, 25, style: PdfFontStyle.bold));
    ...
    //heading
    page.graphics.drawString('Current Problems', PdfStandardFont(PdfFontFamily.timesRoman, 15, style: PdfFontStyle.bold), bounds: const Rect.fromLTWH(0, 100, 0, 0));
    //with heading start with (15,0,0,0)
    page.graphics.drawString('1. Problems/Complains : $controllerNewProblem', PdfStandardFont(PdfFontFamily.timesRoman, 12), bounds: const Rect.fromLTWH(15, 130, 0, 0));
    page.graphics.drawString('2. Symptoms : $controllerSymptoms', PdfStandardFont(PdfFontFamily.timesRoman, 12), bounds: const Rect.fromLTWH(15, 160, 0, 0));
    page.graphics.drawString('3. Treatment/Medications : $controllerMedications', PdfStandardFont(PdfFontFamily.timesRoman, 12), bounds: const Rect.fromLTWH(15, 190, 0, 0));

    List<int> bytes = document.save();
    document.dispose();

    Auth().saveAndLaunchFile(bytes, 'Others.pdf');
  }
}
